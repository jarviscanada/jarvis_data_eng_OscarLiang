...
public class StackFromTwoQueues<E> {
  ...
  /**
   * Time complexity: O(n) Push requires popping all objects from one queue and pushing them to the
   * other, which is n O(1) operations, or O(n).
   */
  ...
  /**
   * Time complexity: O(1) Popping from the head of the queue, which is an O(1) operation
   */
  public E pop() {
    return q2.remove();
  }
